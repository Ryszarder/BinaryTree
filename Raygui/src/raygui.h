...
bool GuiWindowBox(Rectangle bounds, const char *title)
{
    ...
    if (bounds.height < statusBarHeight*2) bounds.height = statusBarHeight*2.0f;
...
Rectangle GuiScrollPanel(Rectangle bounds, Rectangle content, Vector2 *scroll)
{
    ...
    const int horizontalMax = hasHorizontalScrollBar? (int)content.width - (int)bounds.width + verticalScrollBarWidth + GuiGetStyle(DEFAULT, BORDER_WIDTH) - ((GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)? verticalScrollBarWidth : 0) : -GuiGetStyle(DEFAULT, BORDER_WIDTH);
    const int verticalMin = hasVerticalScrollBar? -GuiGetStyle(DEFAULT, BORDER_WIDTH) : -GuiGetStyle(DEFAULT, BORDER_WIDTH);
    const int verticalMax = hasVerticalScrollBar? (int)content.height - (int)bounds.height + horizontalScrollBarWidth + GuiGetStyle(DEFAULT, BORDER_WIDTH) : -GuiGetStyle(DEFAULT, BORDER_WIDTH);
    ...
    if (scrollPos.x > -horizontalMin) scrollPos.x = (float)-horizontalMin;
    if (scrollPos.x < -horizontalMax) scrollPos.x = (float)-horizontalMax;
    if (scrollPos.y > -verticalMin) scrollPos.y = (float)-verticalMin;
    if (scrollPos.y < -verticalMax) scrollPos.y = (float)-verticalMax;
    //--------------------------------------------------------------------
    ...
    if (hasHorizontalScrollBar)
    {
        ...
        GuiSetStyle(SCROLLBAR, SCROLL_SLIDER_SIZE, (((int)bounds.width - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth)/content.width)*(bounds.width - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth));
        scrollPos.x = -GuiScrollBar(horizontalScrollBar, (int)-scrollPos.x, horizontalMin, horizontalMax);
    }
    ...
    if (hasVerticalScrollBar)
    {
        ...
        scrollPos.y = -GuiScrollBar(verticalScrollBar, (int)-scrollPos.y, verticalMin, verticalMax);
    }
...
void GuiDrawIcon(int iconId, Vector2 position, int pixelSize, Color color)
{
    ...
    for (int i = 0, y = 0; i < RICON_SIZE*RICON_SIZE/32; i++)
    {
        for (int k = 0; k < 32; k++)
        {
            ...
            {
            #if !defined(RAYGUI_STANDALONE)
                DrawRectangle(position.x + (k%RICON_SIZE)*pixelSize, (int)(position.y + y*pixelSize), pixelSize, pixelSize, color);
            #endif
            }
